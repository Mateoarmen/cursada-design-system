...
(function () {
  ...
  // tarde") sin impedir usar el resto de la app — si siguen faltando datos,
  // vuelve a aparecer en el próximo login, sin un flag de "no preguntar más"
  // (a diferencia del aviso de importar datos locales, acá si se pospone es
  // a propósito que se repita).
  // Cuentas de Google: obligatorio — Google no permite un formulario propio
  // antes de crear la cuenta, así que estos datos nunca se pidieron todavía;
  // se piden acá, ahora, y no se puede seguir usando la app sin completarlos.
  function maybeOfrecerCompletarPerfil() {
    if (!perfilIncompleto(CURRENT_PROFILE)) return false;
    openPerfilModal(esCuentaGoogle() ? 'completar-obligatorio' : 'completar');
    return true;
  }
