...
(function () {
  ...
  async function wizCrearMateriasAprobadas() {
    ...
    for (var i = 0; i < semestresNecesarios.length; i++) {
      ...
      // Si el usuario ya marcó este número como "lo estoy cursando ahora"
      // (Paso 3, WIZ.semestresElegidos), estas materias van directo al
      // semestre actual en vez de a un histórico aparte — ver
      // obtenerOCrearSemestrePeriodo, que además fusiona cualquier
      // histórico previo con el mismo número dentro del actual.
      semHistoricoPorNumero[n] = WIZ.semestresElegidos.indexOf(n) >= 0 ? WIZ.semestreId : await obtenerOCrearSemestreHistorico(n);
    }
  ...
  async function obtenerOCrearSemestrePeriodo(periodo, semestresElegidos) {
    ...
    for (var i = 0; semestresElegidos && i < semestresElegidos.length; i++) {
      await fusionarHistoricoEnActual(semestresElegidos[i], semestreId);
    }
    return semestreId;
  }
