...
(function () {
  ...
  function agendaBadgeInfo(item, t) {
    if (item.hecho) return { tone: 'success', label: item.itemKind === 'evaluacion' || item.kind === 'evaluacion' ? 'Rendido' : 'Entregado' };
  ...
  async function wizCrearMateriasAprobadas() {
    ...
    var nSemestreActual = numeroSemestreActualElegido(WIZ.semestresElegidos);
    var semHistoricoPorNumero = {};
    for (var i = 0; i < semestresNecesarios.length; i++) {
      var n = semestresNecesarios[i];
      // Sólo el número que nombra al actual (el más alto de los elegidos en
      // Paso 3, ver numeroSemestreActualElegido) va directo al semestre
      // actual — ver obtenerOCrearSemestrePeriodo, que además fusiona
      // cualquier histórico previo con ese mismo número dentro del actual.
      // Un número MÁS BAJO también elegido en Paso 3 (materia arrastrada de
      // un semestre anterior, con horario real vía aplicar_dictados) sigue
      // yendo a su propio histórico acá — mezclarlo en el actual inflaba la
      // lista de "materias de este semestre" con aprobadas/pendientes de
      // otro semestre que no tienen nada que ver con lo que se está
      // cursando ahora (bug reportado: "muestra demasiadas materias").
      semHistoricoPorNumero[n] = n === nSemestreActual ? WIZ.semestreId : await obtenerOCrearSemestreHistorico(n);
    }
  ...
  async function wizContinuar() {
    ...
    if (paso === 'carrera') {
      ...
      try {
        WIZ.semestreId = await obtenerOCrearSemestrePeriodo(PERIODO_ACTUAL, WIZ.semestresElegidos);
      } catch (e) {
        setBtnBusy(btn, false);
  ...
  // en el wizard de onboarding, no de la fecha real del dispositivo. Fallback
  // de nombreSemestreActual() cuando no hay un único número de plan elegido.
  ...
  // De los número/s de plan que el usuario eligió como "lo estoy cursando
  // ahora" (Paso 3 del wizard, WIZ.semestresElegidos), el más alto es el
  // semestre en el que está parado hoy — con 2+ elegidos (p. ej. 2 y 3,
  // porque arrastra una materia de un semestre anterior) los más bajos son
  // sólo lo que arrastra, no el semestre en sí. Usado tanto para nombrar el
  // actual (ver nombreSemestreActual) como para decidir qué histórico
  // fusionar/qué progreso anterior va directo a él (ver
  // fusionarHistoricoEnActual y wizCrearMateriasAprobadas) — los OTROS
  // números elegidos (el 2, en el ejemplo) siguen yendo a su propio
  // histórico, aunque la materia que se está cursando de ese semestre ya
  // tenga horario real en el actual vía aplicar_dictados.
  function numeroSemestreActualElegido(semestresElegidos) {
    return semestresElegidos && semestresElegidos.length ? Math.max.apply(null, semestresElegidos) : null;
  }

  // "Semestre 4" en vez de "2026 · Segundo semestre" — el " · actual" que se
  // ve en Progreso/switcher NO va acá, lo agrega cada vista para el
  // semestre activo, ver los 3 usos de " · actual" más abajo — ponerlo acá
  // lo duplicaría. Sin ningún elegido (0 semestres o wizard hecho a mano),
  // cae al período.
  function nombreSemestreActual(periodo, semestresElegidos) {
    var n = numeroSemestreActualElegido(semestresElegidos);
    return n != null ? 'Semestre ' + n : nombreDesdePeriodo(periodo);
  }

  // Bloque 4 fusiona con lo actual: las materias aprobadas/pendientes que
  // wizCrearMateriasAprobadas() había agrupado en el semestre histórico
  // "Semestre N (antes de Cursada)" en una corrida anterior del wizard son,
  // en realidad, del semestre que el usuario ahora eligió como actual (mismo
  // N) — se pasan al semestre actual y el histórico, si queda vacío, se
  // borra. Sin esto, "materia aprobada de este semestre" quedaba separada de
  // "actual" para siempre (ver comentario del usuario que originó esto).
  async function fusionarHistoricoEnActual(n, semestreActualId) {
    var nombreHistorico = 'Semestre ' + n + ' (antes de Cursada)';
    var historico = loadSemestresRaw().filter(function (s) { return s.historico && s.nombre === nombreHistorico; })[0];
    if (!historico || historico.id === semestreActualId) return;
    var materias = loadMateriasRaw();
    var afectadas = materias.some(function (m) { return m.semestreId === historico.id; });
    if (afectadas) {
      var actualizadas = materias.map(function (m) { return m.semestreId === historico.id ? Object.assign({}, m, { semestreId: semestreActualId }) : m; });
      if (!(await saveMateriasRaw(actualizadas))) throw new Error('No se pudo fusionar el progreso anterior con el semestre actual.');
    }
    var arr = loadSemestresRaw().filter(function (s) { return s.id !== historico.id; });
    if (!(await saveSemestresRaw(arr))) throw new Error('No se pudo fusionar el progreso anterior con el semestre actual.');
  }
  ...
  // la restricción de "un solo activo" vive en la base). semestresElegidos
  // (Paso 3) decide el nombre (ver nombreSemestreActual) y qué históricos
  // fusionar (ver fusionarHistoricoEnActual) — al reusar uno existente sólo
  // se renombra si seguía con el nombre auto-generado por período, para no
  // pisar un nombre que el usuario haya puesto a mano.
  async function obtenerOCrearSemestrePeriodo(periodo, semestresElegidos) {
    var nombreSugerido = nombreSemestreActual(periodo, semestresElegidos);
    ...
    if (existente) {
      ...
      if (existente.nombre === nombreDesdePeriodo(periodo) && nombreSugerido !== existente.nombre) {
        var arrRenombrado = loadSemestresRaw().map(function (s) { return s.id === existente.id ? Object.assign({}, s, { nombre: nombreSugerido }) : s; });
        if (!(await saveSemestresRaw(arrRenombrado))) throw new Error('No se pudo renombrar el semestre.');
      }
      semestreId = existente.id;
    } else {
      var arr = loadSemestresRaw().map(function (s) { return Object.assign({}, s, { activo: false }); });
      semestreId = uid();
      arr.push({ id: semestreId, nombre: nombreSugerido, activo: true, orden: proximoOrdenSemestre(), periodo: periodo });
      if (!(await saveSemestresRaw(arr))) throw new Error('No se pudo crear el semestre.');
    }
    var nActual = numeroSemestreActualElegido(semestresElegidos);
    if (nActual != null) await fusionarHistoricoEnActual(nActual, semestreId);
    return semestreId;
  }
