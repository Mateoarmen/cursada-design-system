...
(function () {
  ...
  // lugar posible para pedir estos datos —y la contraseña, ver
  // perfil-password-group más abajo— es acá, y por eso son obligatorios;
  ...
  function openPerfilModal(modo) {
    ...
    // "Creá una contraseña": únicamente en el gate obligatorio, y sólo si
    // esta cuenta de Google todavía no tiene una (password_set en
    // `profiles` — ver migración profiles_password_set). Una vez seteada
    // queda para siempre: no hay forma de volver a ver este bloque desde
    // el perfil normal (ver comentario en app.html).
    var pedirPassword = obligatorio && !(CURRENT_PROFILE && CURRENT_PROFILE.password_set);
    document.getElementById('perfil-password-group').classList.toggle('hidden', !pedirPassword);
    document.getElementById('perfil-password-nueva').required = pedirPassword;
    document.getElementById('perfil-password-confirmar').required = pedirPassword;
  ...
  function bindProfileUI() {
    ...
    document.getElementById('form-perfil').addEventListener('submit', async function (ev) {
      ...
      // "Creá una contraseña" (ver openPerfilModal): sólo visible/requerido
      // en el gate obligatorio de cuentas Google sin password_set. Se
      // valida acá mismo (coincide + longitud) antes de tocar la red, igual
      // que el resto del formulario — required en el input ya frena el
      // submit si están vacíos.
      var pideNuevaPassword = !document.getElementById('perfil-password-group').classList.contains('hidden');
      var passErrEl = document.getElementById('perfil-password-error');
      passErrEl.classList.add('hidden');
      if (pideNuevaPassword) {
        var passNueva = document.getElementById('perfil-password-nueva').value;
        var passConfirm = document.getElementById('perfil-password-confirmar').value;
        if (passNueva !== passConfirm) {
          passErrEl.textContent = 'Las contraseñas no coinciden.';
          passErrEl.classList.remove('hidden');
          return;
        }
        if (passNueva.length < 6) {
          passErrEl.textContent = 'La contraseña tiene que tener al menos 6 caracteres.';
          passErrEl.classList.remove('hidden');
          return;
        }
      }
      ...
      if (pideNuevaPassword) patch.password_set = true;
      var btn = document.getElementById('btn-perfil-guardar');
      setBtnBusy(btn, true, 'Guardando…');
      // auth.updateUser() primero, en su propio try: si falla (ej. Supabase
      // exige más de 6 caracteres), se corta acá con el error puesto en el
      // campo de contraseña — no llega a tocar `profiles` con
      // password_set:true si en verdad no se pudo setear.
      if (pideNuevaPassword) {
        try {
          var pwRes = await sb().auth.updateUser({ password: document.getElementById('perfil-password-nueva').value });
          if (pwRes.error) throw pwRes.error;
        } catch (e) {
          passErrEl.textContent = traducirErrorAuth(e);
          passErrEl.classList.remove('hidden');
          setBtnBusy(btn, false);
          return;
        }
  ...
  async function onSignedIn(user) {
    ...
    // Cuentas de Google con el perfil sin completar, o sin contraseña
    // propia todavía (password_set en `profiles` — ver migración
    // profiles_password_set): se piden esos datos ANTES de revelar #app —
    // es una pantalla más (fondo sólido, mismo tratamiento que el login),
    // no un modal encima del dashboard. Se espera acá a que la Promise se
    // resuelva (al guardar) antes de seguir.
    if (esCuentaGoogle() && (perfilIncompleto(CURRENT_PROFILE) || !CURRENT_PROFILE.password_set)) {
